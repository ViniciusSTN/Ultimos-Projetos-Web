...
describe('ShoppingCart', () => {
  ...
  it('should call discount.calculate once when totalWithDiscount is called', () => {
    ...
    expect(discountMockSpy).toHaveBeenCalledTimes(1); // quando totalWithDiscount for chamado, discountMockSpy deve ser executado uma vez, ou seja, a função calculate deve ser chamada
  });
  ...
  it('should call discount.calculate with total price when totalWithDiscount is called', () => {
    ...
    const discountMockSpy = jest.spyOn(discountMock, 'calculate');
    sut.totalWithDiscount();
    expect(discountMockSpy).toHaveBeenCalledWith(sut.total()); // se calculate vai ser chamado com o método sut.total()
  });
});
